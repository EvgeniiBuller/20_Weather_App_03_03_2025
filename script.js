const API_KEY = "c930ace00e13a5256655b4266a3d02e7";
    ...
    fetch(`${BASE_URL}?q=${cityName}&appid=${API_KEY}&units=metric`)
      .then((response) => {
        if (response.status) {
          ...
          throw new Error(
            "Проверти правильность написания населённого пункта."
          );
      })
      .then(
        ({
          ...
          const sunsetSecondes =
            sunsetDate.getSeconds() < 10
              ? "0" + sunsetDate.getSeconds()
              : sunsetDate.getSeconds();

          //https://openweathermap.org/img/wn/10d@2x.png (иконка отображает картинку о погоде)

          weatherContainer.innerHTML = `
                <dif class="d-flex justify-content-center align-items-center">
                <h2>${name}</h2>
                ...
                <p>Sunrise: ${sunriseHours} : ${sunriseMinutes} : ${sunriseSecondes}</p>
                <p>Sunset: ${sunsetHours} : ${sunsetMinutes} : ${sunsetSecondes}</p>
                `;
